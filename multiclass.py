...
class MCTree:
    ...
    def train(self, X, Y):
        for n in self.tree.iterNodes():
            ...
            ### TODO: YOUR CODE HERE
            ### here we form 2 array thisX and thisY 
            ### if we have the label Y[i] in leftLabels than we assign X[i]--->-1
            ### if we have the label Y[i] in rightLabels than we assign X[i]--->1
            thisX=[]
            thisY=[]
            for i in range(len(Y)):
                if (leftLabels.count(Y[i])>0):
                    thisX.append(X[i])
                    thisY.append(-1)
                elif(rightLabels.count(Y[i])>0):
                    thisX.append(X[i])
                    thisY.append(1)
            try:
    ...
    def predict(self, X):
        ...
        while(t.isLeaf==False):
            probs=t.getNodeInfo().predict_proba(X)
            if(probs[0,1]>0.5):
                t=t.getRight()
            else:
                t=t.getLeft()
