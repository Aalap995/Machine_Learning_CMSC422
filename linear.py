...
class LogisticLoss(LossFunction):
    ...
    def lossGradient(self, X, Y, Yhat):
        ...
        A = []
        for i in range (len(power)):
            A.append(exp(power[i]))
        B = []
        for i in range (len(A)):
            B.append((A[i]/(1+A[i])))        
        C = B*Y
        return - sum((C) * X.T, axis=1)
    ...
    def train(self, X, Y):
        ...
        def func(w):
            ...
            for i in range (len(X)):
                Yhat.append(sum(self.weights*X[i]))   ### TODO: YOUR CODE HERE

            obj  = lossFn.loss(Y,Yhat)+(lambd/2)*norm(self.weights)**2    ### TODO: YOUR CODE HERE
        ...
        def grad(w):
            ...
            gr   = lossFn.lossGradient(X,Y,Yhat) + lambd*self.weights    ### TODO: YOUR CODE HERE
